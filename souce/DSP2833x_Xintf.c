...
void InitXintf(void)
{
    ...
    // All Zones---------------------------------
    EALLOW;
    XintfRegs.XINTCNF2.bit.XTIMCLK = 1; // Timing for all zones based on XTIMCLK = 1/2 SYSCLKOUT
    XintfRegs.XINTCNF2.bit.WRBUFF = 0;  // No write buffering
    XintfRegs.XINTCNF2.bit.CLKOFF = 0;  // XCLKOUT 使能
...
*/
    ...
    // Lead must always be 1 or greater
    // Zone write timing
    XintfRegs.XTIMING6.bit.XWRLEAD = 3;//写访问的建立时间中等待周期设定位
    XintfRegs.XTIMING6.bit.XWRACTIVE =7;//写访问有效时间等待周期的设定位
    XintfRegs.XTIMING6.bit.XWRTRAIL = 3;//写访问有效跟踪时间等待周期的设定位
    // Zone read timing
    XintfRegs.XTIMING6.bit.XRDLEAD = 3;//读访问的建立时间中等待周期设定位
    XintfRegs.XTIMING6.bit.XRDACTIVE = 7;//读访问有效时间等待周期的设定位
    XintfRegs.XTIMING6.bit.XRDTRAIL = 3;//读访问有效跟踪时间等待周期的设定位

    // double all Zone read/write lead/active/trail timing
    XintfRegs.XTIMING6.bit.X2TIMING = 1;//该位设定比值

    // Zone will sample XREADY signal
    XintfRegs.XTIMING6.bit.USEREADY = 0;
    XintfRegs.XTIMING6.bit.READYMODE = 0;  // sample asynchronousTONGBU0 YIBU1

    // Size must be either:
    // 0,1 = x32 or
    // 1,1 = x16 other values are reserved
    XintfRegs.XTIMING6.bit.XSIZE = 3;//数据总线宽度
    ...
    // Lead must always be 1 or greater
    // Zone write timing
    XintfRegs.XTIMING7.bit.XWRLEAD = 3;   //写访问的建立时间中等待周期设定位
    XintfRegs.XTIMING7.bit.XWRACTIVE = 7; //写访问有效时间等待周期的设定位
    XintfRegs.XTIMING7.bit.XWRTRAIL = 3;  //写访问有效跟踪时间等待周期的设定位
    // Zone read timing
    XintfRegs.XTIMING7.bit.XRDLEAD = 3;   //读访问的建立时间中等待周期设定位
    XintfRegs.XTIMING7.bit.XRDACTIVE = 7; //读访问有效时间等待周期的设定位
    XintfRegs.XTIMING7.bit.XRDTRAIL = 3;  //读访问有效跟踪时间等待周期的设定位


    // double all Zone read/write lead/active/trail timing
    XintfRegs.XTIMING7.bit.X2TIMING = 1;//该位设定比值
    ...
    // 0,1 = x32 or
    // 1,1 = x16 other values are reserved
    XintfRegs.XTIMING7.bit.XSIZE = 3;//数据总线宽度
    ...
    XintfRegs.XBANK.bit.BANK = 6;//指定切换等待区域位
    XintfRegs.XBANK.bit.BCYC = 7;//等待周期
    EDIS;
...
//   GpioCtrlRegs.GPBMUX1.bit.GPIO35 = 3;// XRNW
  ...
  // GpioCtrlRegs.GPBMUX2.bit.GPIO36 = 3;  // XZCS0
